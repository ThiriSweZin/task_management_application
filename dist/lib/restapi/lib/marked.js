 ...
 * Helpers
 */
const escaper = function (html, encode = false) {
    if (encode) {
        if (escaper.escapeTest.test(html)) {
            return html.replace(escaper.escapeReplace, (ch) => { return escaper.replacements[ch]; });
        }
    }
    else {
        if (escaper.escapeTestNoEncode.test(html)) {
            return html.replace(escaper.escapeReplaceNoEncode, (ch) => { return escaper.replacements[ch]; });
        }
    }
    return html;
};
...
function rtrim(str, c, invert = false) {
    if (str.length === 0) {
        return "";
    }
